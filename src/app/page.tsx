...
import { PortfolioResponse, User, UserObject } from "@/utils/interfaces";
import Experience from "@/components/experience";
import { ContactUs } from "@/components/contact-us";
import Link from "next/link";
import { Hero } from "@/components/hero";
import Image from "next/image";
import Logo from '../../public/portfolio-logo.webp';

export default async function Home() {
  const res = await fetch(
    "https://portfolio-backend-srt8.onrender.com/api/v1/get/user/65b3a22c01d900e96c4219ae"
  );
  ...
  return (
    <main className="relative">
      <Transition className="fixed md:top-8 top-6 md:left-8 left-6 z-30 hover:text-white/80 text-white/40">
        <Link href={"/"}>
          {/* <TextReveal className="font-semibold ">ManujVision</TextReveal> */}
            <Image
              src={Logo}
              width={200}
              alt={about?.name}
